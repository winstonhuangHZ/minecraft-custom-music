...
1. 装好 Fabric Loader，把 `custom-music-1.1.0.jar` 丢进 `mods/`（Fabric API 不需要，ModMenu 可选）
...
情境界面最上面是**分组预设**（前面标着「组」），一次给一整组分配，不用一个个点：

| 分组 | 包含 | 数量 |
|---|---|---|
| 全部情境 | 所有配乐事件 | 32 |
| 洞穴 | 繁茂洞穴 / 溶洞 / 深暗之域 / 硫磺洞穴 | 4 |
| 下界 | 下界荒地 / 灵魂沙峡谷 / 玄武岩三角洲 / 绯红森林 / 诡异森林 | 5 |
| 主世界地表 | 除洞穴以外的全部主世界群系 | 16 |
| 末地 / 水下 / 主菜单 / 创造模式 | 各一个 | 1 |

分组分配之后仍然是**按情境存**的，所以你完全可以先整组分配「下界 = 某张专辑」，
再单独把「下界 · 玄武岩三角洲」改成别的。
